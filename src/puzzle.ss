...
(def *class* (make-vector (+ typemax 1) 0))
...
(def (place (i : :fixnum) (j : :fixnum))
  ...
  (let (end (u16vector-ref *piecemax* i))
    ...
    (u16vector-set! *piececount*
                   (vector-ref *class* i)
                   (fx- (u16vector-ref *piececount* (vector-ref *class* i)) 1))
    ...
    (call-with-current-continuation
     (lambda ((return :- :procedure))
       (do (((i :- :fixnum) 0 (fx+ i 1)))
         ...
         (unless (fx= 0 (u16vector-ref *piececount* (vector-ref *class* i)))
           (cond
            ((fit i j)
             (set! k (place i j))
             (cond
...
(def (definePiece iclass (ii : :fixnum) (jj : :fixnum) (kk : :fixnum))
  (let ((index 0))
    (do (((i :- :fixnum) 0 (fx+ i 1)))
        ((fx> i ii))
